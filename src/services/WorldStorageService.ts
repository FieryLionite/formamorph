...
class WorldStorageService {
          ...
          try {
            if (request.result) {
              ...
              if (!request.result.data || typeof request.result.data !== 'object') {
                ...
                reject('Invalid world data format: Missing data object');
                return;
              }
              
              const requiredFields = ['worldOverview', 'stats', 'locations', 'entities', 'traits', 'statUpdates'];
              const missingFields = requiredFields.filter(field => !(request.result.data as any)[field]);
              
              if (missingFields.length > 0) {
                console.error('Missing required fields in data:', {
                  worldOverview: !!(request.result.data as any).worldOverview,
                  stats: !!(request.result.data as any).stats,
                  locations: !!(request.result.data as any).locations,
                  entities: !!(request.result.data as any).entities,
                  traits: !!(request.result.data as any).traits,
                  statUpdates: !!(request.result.data as any).statUpdates
                });
                reject(`Invalid world data format: Missing required fields: ${missingFields.join(', ')}`);
                return;
              }
              
              // Validate that arrays are actually arrays
              const arrayFields = ['stats', 'locations', 'entities', 'traits', 'statUpdates'];
              for (const field of arrayFields) {
                if (!Array.isArray((request.result.data as any)[field])) {
                  console.error(`Field '${field}' is not an array`);
                  reject(`Invalid world data format: '${field}' must be an array`);
                  return;
                }
              }
            ...
            console.error('Error validating world data:', validationError);
            reject(`Corrupted world data: ${(validationError as Error).message}`);
          }
        };
  ...
  async fetchRemoteWorlds(page = 1, limit = 10, search = '', ownedOnly = false, searchByAuthor = false, sort = '', order = 'desc') {
    try {
      ...
      // Default sorting: if no explicit sort was applied and we're looking at user worlds,
      // order them by newest update
      if (ownedOnly && !sort && Array.isArray(responseData.data)) {
  ...
  async setWorldSpoilerStatus(worldId: string, spoiler: boolean) {
    if (!AuthService.isAuthenticated()) {
      throw new Error('You must be logged in to update spoiler status');
    }
    
    try {
      const response = await fetch(`${this.API_URL}/worlds/${worldId}/spoiler`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${AuthService.token}`
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || 'Failed to update spoiler status');
      }
      
      return await response.json();
    } catch (error) {
      console.error('Error updating spoiler status:', error);
      throw error;
    }
  }
